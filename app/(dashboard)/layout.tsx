...
const DashboardLayout = ({ children }) => {
  return (
    <div className="w-screen h-screen relative">
      <aside className="absolute left-0 top-0 h-full w-[250px] border-r border-black/10 bg-gray-100">
        <div className="px-4 my-4 text-center">
          <Link
            href="/"
            className="text-4xl font-extrabold text-blue-600 cursor-pointer hover:text-blue-800 transition-colors duration-150 ease-in-out"
          >
            Mynd
          </Link>
        </div>
          ...
          <ul className="px-4 mt-2">
            {links.map((link) => (
              <li key={link.name} className="my-2">
                <Link
                  href={link.href}
                  className="flex items-center px-3 py-2 rounded-md text-gray-700 hover:bg-gray-200 transition-colors duration-150 ease-in-out"
                >
                  {link.name === 'Journals' && (
                    <svg
      ...
      <div className="ml-[250px] h-full w-[calc(100vw-250px)]">
        <header className="h-[60px] border-b border-black/10 bg-gray-100">
